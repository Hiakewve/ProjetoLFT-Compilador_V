# 👨🏼‍💻 Projeto de Compiladores em V
Projeto desenvolvido na disciplina Linguagens Formais e Tradutores (LFT), ministrada pelo professor Doutor André Luiz Menezes.  
O objetivo é aprender e aplicar conceitos da disciplina, explorando desde a análise de linguagens até a implementação prática em V.

## 🚀 Tecnologias
- V
- Python

## 👥 Integrantes
- 👨‍💻 [Hiakewve Santos Alves](https://github.com/Hiakewve)
- 👨‍💻 [Miguel Menezes Andrade](https://github.com/Zerxf-exe)
- 👨‍💻 [Vinicius Meneses Oliveira](https://github.com/viniciusmeneses159)

**Professor:** [André Luiz Menezes](https://github.com/andreluisms)

## 📋 Passos a seguir

1. **Atividade 1**: Elaboração da Documentação Léxica.
2. **Atividade 2**: Elaboração do Léxico em PLY.
3. **Atividade 3**: Elaboração da Documentação Sintática e Exemplos de Código.
4. **Atividade 4**: Elaboração da Gramática Livre de Contexto e implementação no PLY
    - GLC
    - Implementação no PLY
5. **Atividade 5**: Elaboração da Sintaxe Abstrata e Visitor Pretty Printer
    - Exemplo Sintaxe Abstrata
    - Exemplo Visitor ImprimeCodigo
6. **Atividade 6**: Análise Semântica
    - Exemplo Análise Semântica
7. **Atividade 7**: Geração de Assembly.

8. Teste de GitHub